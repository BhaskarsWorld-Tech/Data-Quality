...
async function testSnowflake(conn: Record<string, unknown>): Promise<TestResult> {
  ...
  try {
    ...
    if (loginRes.status === 200 && loginBody?.success === true) {
      ...
      await store.connections.update(conn.id as string, { status: 'active', lastTested: new Date().toISOString() })
      ...
      if (sfMessage.toLowerCase().includes('incorrect username or password') || sfCode === '390100') {
        ...
        await store.connections.update(conn.id as string, { status: 'error', lastTested: new Date().toISOString() })
        return {
          success: false, status: 'error', steps,
...
async function testGeneric(conn: Record<string, unknown>, type: string): Promise<TestResult> {
  ...
  await store.connections.update(conn.id as string, { status:'inactive', lastTested: new Date().toISOString() })
  return {
...
export async function POST(req: NextRequest) {
  ...
  const connection = await store.connections.getById(connectionId)
  if (!connection) {
