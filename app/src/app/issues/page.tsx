...
const INIT: Issue[] = [
  ...
  },
  {
    ...
    impact: 'Inter-company eliminations missing from the ledger will cause the consolidated P&L to overstate gross revenue. Month-end close at risk if not resolved before EOD.',
  ...
  },
  {
    ...
    impact: 'Revenue was double-counted for ~$180K. Resolved by deduplication and Redis failover implementation.',
  ...
  },
  {
    ...
    rootCause: 'A currency conversion rate for JPY was entered as 0.0073 instead of 0.0073 USD/JPY — a trailing-zero data entry error caused ~340 transactions to show revenue 10× higher than expected.',
...
const sevCfg = {
  ...
  warning:  { bg: '#fef3c7', color: '#ea8b3a', dot: '#ea8b3a', label: 'Warning'  },
}
const stCfg = {
...
export default function IssuesPage() {
  ...
  function setStatus(id: string, newStatus: Status) {
  ...
  const statCards = [
    { key: 'open'          as 'all'|Status, label: 'Open',          value: open, color: '#dc2626', bg: '#fee2e2', border: '#fca5a5' },
    { key: 'investigating' as 'all'|Status, label: 'Investigating', value: inv,  color: '#ea8b3a', bg: '#fef3c7', border: '#fcd34d' },
    { key: 'resolved'      as 'all'|Status, label: 'Resolved',      value: res,  color: '#16a34a', bg: '#dcfce7', border: '#86efac' },
    { key: 'all'           as 'all'|Status, label: 'Critical',      value: crit, color: '#dc2626', bg: '#fff1f2', border: '#fda4af' },
  ]
  ...
  return (
    <div style={{ padding: '28px 36px', maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '20px' }}>
        {statCards.map(s => {
          const active = statusF === s.key && s.key !== 'all'
          return (
            <button key={s.label} onClick={() => setStatusF(active || s.key === 'all' ? 'all' : s.key)} style={{
              background: s.bg, border: `2px solid ${active ? s.border : 'transparent'}`,
              borderRadius: '12px', padding: '16px 18px', cursor: 'pointer', textAlign: 'left',
              boxShadow: active ? `0 0 0 3px ${s.border}50` : 'none', transition: 'all 0.15s',
            }}>
              <div style={{ fontSize: '11.5px', color: '#64748b', marginBottom: '6px', fontWeight: active ? 700 : 400 }}>{s.label}</div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        {(['all', 'critical', 'warning'] as const).map(s => (
          <button key={s} onClick={() => setSevF(s)} style={{
            ...
            borderColor: sevF === s ? (s === 'critical' ? '#dc2626' : s === 'warning' ? '#ea8b3a' : '#1a1a1a') : '#e2e8f0',
            background: sevF === s ? (s === 'critical' ? '#fee2e2' : s === 'warning' ? '#fef3c7' : '#1a1a1a') : '#fff',
            color: sevF === s ? (s === 'critical' ? '#dc2626' : s === 'warning' ? '#ea8b3a' : '#fff') : '#475569',
        ...
        {filtered.map(issue => {
          ...
          return (
            <div key={issue.id} style={{
              background: '#fff',
              border: `1.5px solid ${isOpen ? '#6366f1' : issue.status === 'resolved' ? '#d1fae5' : issue.severity === 'critical' ? '#fca5a5' : '#fde68a'}`,
              borderRadius: '14px', overflow: 'hidden',
              boxShadow: isOpen ? '0 4px 20px rgba(99,102,241,0.12)' : '0 1px 3px rgba(0,0,0,0.04)',
              transition: 'all 0.2s',
            }}>
              ...
              <div onClick={() => setExpanded(isOpen ? null : issue.id)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    <span style={{ fontSize: '11.5px', color: '#94a3b8', fontFamily: 'monospace', fontWeight: 600 }}>{issue.id}</span>
                    <span style={{ background: sc.bg, color: sc.color, padding: '1px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 700 }}>{sc.label}</span>
                  </div>
                  <div style={{ fontSize: '13.5px', fontWeight: 700, color: '#1a1a1a', marginBottom: '4px' }}>{issue.title}</div>
                  <div style={{ fontSize: '12px', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                    ...
                    {issue.count > 0 && <span style={{ color: sc.color, fontWeight: 600 }}>· {issue.count.toLocaleString('en-US')} records affected</span>}
                    {issue.count === 0 && <span style={{ color: '#16a34a', fontWeight: 600 }}>· resolved</span>}
                  </div>
                </div>

                <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>
                  <span style={{ background: st.bg, color: st.color, padding: '3px 12px', borderRadius: '20px', fontSize: '11.5px', fontWeight: 700 }}>{st.label}</span>
                  <span style={{ fontSize: '11.5px', color: '#94a3b8' }}>{issue.owner} · {issue.opened}</span>
                </div>

                <div style={{ fontSize: '18px', color: '#94a3b8', flexShrink: 0 }}>{isOpen ? '▲' : '▼'}</div>
              </div>
              ...
              {isOpen && (
                <div style={{ borderTop: '1px solid #f1f5f9', padding: '20px 24px', background: '#fafafa' }}>
                  ...
                  {/* Status workflow + Edit actions */}
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
                    <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b', marginRight: '4px' }}>Change status:</span>
                    {nextStatuses.map(ns => (
                      <button key={ns} onClick={e => { e.stopPropagation(); setStatus(issue.id, ns) }} style={{
                        padding: '6px 14px', borderRadius: '8px', border: `1.5px solid ${stCfg[ns].color}`,
                        background: '#fff', color: stCfg[ns].color, fontSize: '12px', fontWeight: 700,
                    ...
                    <button onClick={e => { e.stopPropagation(); openEdit(issue) }} style={{
                      ...
                      background: '#f5f3ff', color: '#6366f1', fontSize: '12px', fontWeight: 700, cursor: 'pointer',
                    }}>✏️ Edit Issue</button>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
                    {/* Root cause */}
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '14px', border: '1px solid #e9eef5' }}>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: '#6366f1', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>🔍 Root Cause</div>
                      <div style={{ fontSize: '12.5px', color: '#374151', lineHeight: '1.6' }}>{issue.rootCause}</div>
                    </div>
                    {/* Impact */}
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '14px', border: `1px solid ${sc.color}30` }}>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: sc.color, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>⚡ Business Impact</div>
                      <div style={{ fontSize: '12.5px', color: '#374151', lineHeight: '1.6' }}>{issue.impact}</div>
                    </div>
                  </div>

                  {/* Recommendation */}
                  <div style={{ background: '#f0fdf4', borderRadius: '10px', padding: '14px', border: '1px solid #bbf7d0', marginBottom: '16px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#16a34a', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>✅ Recommended Fix</div>
                    <div style={{ fontSize: '12.5px', color: '#374151', lineHeight: '1.6' }}>{issue.recommendation}</div>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                    {/* Affected columns */}
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '14px', border: '1px solid #e9eef5' }}>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>📋 Affected Columns</div>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {issue.affectedColumns.map(c => (
                          <code key={c} style={{ background: '#f1f5f9', color: '#475569', padding: '3px 8px', borderRadius: '6px', fontSize: '11.5px', fontFamily: 'monospace' }}>{c}</code>
                        ))}
                      </div>
                    </div>
                    {/* Activity log */}
                    <div style={{ background: '#fff', borderRadius: '10px', padding: '14px', border: '1px solid #e9eef5' }}>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>🕐 Activity</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', maxHeight: '120px', overflowY: 'auto' }}>
                        {[...issue.activity].reverse().map((a, i) => (
                          <div key={i} style={{ display: 'flex', gap: '8px', fontSize: '11.5px' }}>
                            <span style={{ color: '#94a3b8', flexShrink: 0 }}>{a.time}</span>
                            <span style={{ color: '#6366f1', fontWeight: 600, flexShrink: 0 }}>{a.by}</span>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, backdropFilter: 'blur(4px)' }}
          ...
          <div style={{ background: '#fff', borderRadius: '16px', width: '480px', boxShadow: '0 24px 64px rgba(0,0,0,0.2)', overflow: 'hidden' }}
